...
from engine.enums import ActionType, CardType, Keyword, SubType, SuperType, Zone
    ...
    if actual_damage > 0:
        ...
        if ctx.ability_registry is not None:
            ...
            if on_hit_handler is not None:
                from engine.rules.abilities import AbilityContext as _AC
                on_hit_ctx = _AC(
                    state=ctx.state,
                    source_card=dagger,
...
class MaskOfDeceitTrigger(TriggeredEffect):
    ...
    def create_triggered_event(self, triggering_event: GameEvent) -> GameEvent | None:
        ...
        if attacker.is_marked:
            ...
            from engine.enums import DecisionType
