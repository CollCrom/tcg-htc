...
def grant_power_bonus(
    ...
    """Grant +N power to a target card via end-of-combat continuous effect."""
...
def grant_keyword(
    ...
    """Grant a keyword to a target card via end-of-combat continuous effect."""
...
def create_token(
    ...
    *,
    ...
    if event_bus is not None and effect_engine is not None:
        from engine.cards.abilities.tokens import register_token_triggers
...
def choose_dagger(
    ...
    *,
    ...
    options = [
        ActionOption(
            action_id=f"dagger_{c.instance_id}",
            description=f"{c.name} (ID {c.instance_id})",
    ...
    return next(
        (c for c in daggers if c.instance_id == chosen_id), daggers[0]
    )
