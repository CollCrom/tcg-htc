...
### 2026-04-04: TODO cleanup (PR #88)

- **24 items addressed** from orchestrator memory, skeptic memory, and code TODOs.
- **Bug fixes**: Frailty -1 power now arsenal-only (added `played_from_zone` on CardInstance), Contract keyword converted to global BANISH trigger, Hunter's Klaive duplicate on_hit deduped.
- **Dead code removed**: `Layer.has_go_again`, `_is_draconic_attack` in equipment.py, stale `_is_keyword_inherent` docstring.
- **definition.X bypasses fixed**: Phantasm, `_is_assassin_attack`, Ancestral Empowerment, Authority of Ataya now use effect engine. New `get_modified_subtypes` infrastructure added.
...
- **Trigger processing**: Added `_process_pending_triggers()` after DRAW_CARD and CREATE_TOKEN events.
- **15 new tests**, 959 total passing. Skeptic: CLEAN after 2 rounds.
...
- Contract trigger accumulation is correct per FaB rules — each card with Contract generates an independent continuous effect. Multiple copies = multiple Silver tokens per banish.
- Stealth checks in target_filters use `_effective_definition.keywords` (not full effect engine) to avoid recursive `_resolved_supertypes` conflicts. Acceptable since no effects grant Stealth.
